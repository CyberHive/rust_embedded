...
use core::ffi::{
    c_char, c_int, c_longlong, c_schar, c_short, c_uchar, c_uint, c_ulonglong, c_ushort, c_void,
};
// Rust bindings for LwIP TCP/IP stack.
// These should accompany a wrapper for the Rust bindings generated by bindgen
include!("lwip-rs.rs");
...
pub mod netc {
    use super::{ip_addr_t, /*pbuf,*/ tcp_pcb, udp_pcb};
    use super::{lwip_htons, pbuf_alloc, pbuf_free, tcp_bind, udp_bind, /*udp_send,*/ udp_sendto,};
    use super::{pbuf_layer_PBUF_TRANSPORT, pbuf_type_PBUF_RAM};
    ...
    // TODO: Caller in sys_common explicitly uses constants in netc scope. What to do about duplicates (eg. IPPROTO_IP) which are also
    // defined in lwip Rust bindings?
    pub const AF_INET6: i32 = 10;
    pub const AF_INET: i32 = 2;
    pub const IPPROTO_IP: i32 = 0;
    pub const IPPROTO_IPV6: i32 = 41; //Not sure if supported in LwIP
    ...
    pub fn bind(sock: RawSocket, name: *const sockaddr, namelen: socklen_t) -> c_int {
        match sock.socket_type {
            SOCK_DGRAM => {
                let retval = unsafe {
                    udp_bind(
                        ...
                        &((*name).ipaddr),
                        lwip_htons((*name).port),
                    ) // Not expecting htons to be necessary here. Further investigation needed!
                };
                match retval {
                    0 => 0,
                    _ => -1,
            ...
            SOCK_STREAM => {
                let retval = unsafe {
                    tcp_bind(
                        sock.socket_handle as *mut tcp_pcb,
                        &((*name).ipaddr),
                        lwip_htons((*name).port),
                    ) // Not expecting htons to be necessary here. Further investigation needed!
                };
    ...
    pub fn sendto(
        ...
        //TODO: Add a check for len > 65535
...
impl Socket {
    ...
    pub fn new(addr: &SocketAddr, socket_type: c_int) -> io::Result<Socket> {
        ...
        match socket_type {
            SOCK_DGRAM => {
                let udp_pcb = unsafe { udp_new() }; // returns NULL on error
                if udp_pcb.is_null() {
            ...
            _ => Err(io::const_io_error!(io::ErrorKind::InvalidInput, "Unknown socket type")),
        }
    }
